...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
In this paper we set out to investigate wether expectations about user searching behaviour raised by the Optimal Foraging Theory can be met in an environment that \textit{simulates} scenarios  requiring decisions and
actions associated with information foraging tasks.

The environment chosen is a game: Gold Digger. In this game users can dig through mines and acquire gold, as a metaphor for searching and acquiring useful information. In Gold Digger, users can also purchase items to acquire more gold and dig or move at a lower cost, simulating 'enrichment strategies' of within and between patch enricment.

Users were able to play this game as they pleased, with no instructions as to what the optimal behaviour should be. A total of 39 users/players entered the experiment/game over a three week period. At the end of this period, our findings seem to match expectations. Users move from one mine to the other when the first one stops being productive at the right time with an absolute discrepancy of only one layer. Furthermore there seem to be relationships indicating that the ability to see clues on the amount of gold in each layer before it is dug also influence the user's behaviour.

These findings, in turn, seem to confirm that users behave in an environment that is merely a metaphor, in t the same as they would if looking for information.
...
Finally, I would like to thank all of the users who played Gold Digger and in particular Martin Crean for his advice on game features and Martyna Maron. 
...
And that (2)âthe total amount of within-patch time can be represented as:
...
\textit{Page Fetch} and its second iteration \textit{Page Fetch 2} however, have a more complicated scoring system which introduces game dynamics that bring us closer to Gold Digger. In \textit{Page Fetch 2}, in fact, the user is encouraged to enter the shortest possible query in order to retrieve a particular page. This is done in order to discourage players from entering an obvious long query and instead be strategic, since the shortest query will yield the most points. In a similar way, in Gold Digger, the user that manages to dig least amount of layers while getting the greatest payoff is rewarded. 
 ...
 \textit{Page Fetch 2} also introduces a series of game features that are specifically aimed at enhancing the user experience and extending the amount of time users spend playing the game. These features include the awarding of badges and the ranking of players in leaderboards, so that they can compare their performance against other players and strive to be the best. In addition to this, \textit{Page Fetch 2}, allows the user to choose between different categories of similarly themed web pages. In the same way, Gold Digger offers the user a series of different locations in which to play.Gold Digger also incorporates these features (see ~\ref{subsec:achievements}) and tries to expand them further in the direction of an even more enhanced gaming experience. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\chapter{Design}\label{design}

The design of Gold Digger went through different iterations through the course of its development (see 4.1 Development Methods), however some design choices were made in order to keep the user experience consistent throughout the website and enhance clarity and ease of use.
...
On the landing page (fig \ref{fig:homepage}) users will be presented with a short \textbf {five-point explanation} of the game to quickly explain the mechancs of the game so that
...
picture. Finally, the modal displays a disclaimer in order to both make sure that the users are aware of the limitations of accessing the website and of its purpose. 
...
\begin{figure} [h] 
	...
	\caption{The world map displays six locations with a pricetag representing the amount of gold players will need to pay in order to enter}
           \label{worldmap}
\end{figure}

Once users have registered or have been logged in, they are automatically redirected to the world map so that they can start playing immediately. 
On the left of the page, users can see the side item panel and the amount of gold they have at the moment. The side item panel appears on the left column of the page, once the user has logged in. The panel shows the items that users have equipped, together with the total amount of gold in their possession. By hovering over each one of the items a tooltip will appear, showing the essential stats of each item. The presence of the side item panel has the function of both confirming the users that they are logged in and  reminding them that their game session has started  contributing to the uniformity of the user experience.
\begin{figwindow}%
[0, l, \includegraphics[width=.4\textwidth]{cali.png},%
{ California modal. California is the cheapest mine, it costs only 40 gold to dig here.}]
...
but it also has a potentially higher gold yield. It is up to the player to enter the right mine at the right time.
\end{figwindow}
...
\begin{figure}[h] 
	...
	\caption{A Californian mine where the first two layers have been dug. The buttons are displayed on the right hand side next to the first undug layer.}
           \label{mine}
\end{figure}
...
\begin{figure} [h] 
	...
	\caption{Leaderboards diplay all users ranked in 5 diefferent categories. Only the achievements table in not ranked.}
...
Achievements and special achievements have been incorporated in order to both add an extra game feature to Gold Digger which wouldn't change the game mechanics too much and at the same time to give players an incentive to keep playing, especially since there is no particular "winning condition", fulfilled which a player has completed the game.  
...
(3) Jill decides to dig through the first couple of layers of the mine (fig~\ref{fig:mine}) by clicking on the 'Move' button on the right hand side of the mine shaft. She gains 15 gold nuggets and consumes 10 units of time (since each digging operation costs 5 units of time)
\begin{itemize}
...
(4) At this point Jill is not sure she might get much more gold from this mine and she doesn't want to spend precious time digging through layers of a mine that she has no information about. For this reason she clicks on 'Move' to get to a new mine (in the same location). To do this she must spend 10 time.
\begin{itemize}
	\item \textbf{Current gold}: 15
...
[1]	\textbf{Azzopardi}, Leif, Diane Kelly, and Kathy Brennan. "How query cost affects search behavior."\textit{Proceedings of the 36th international ACM SIGIR conference on Research and development in information retrieval}. ACM, 2013.

[2]	\textbf{Charnov} Eric L. "Optimal foraging, the marginal value theorem." \textit{Theoretical population biology} 9.2 (1976): 129-136.

[3]	\textbf{Pirolli} Peter, and Stuart Card. "Information foraging." \textit{Psychological review} 106.4 (1999): 643.

[4]	\textbf{Thomas}, Paul, Falk Scholer, and Alistair Moffat. "Fading away: Dilution and user behaviour." \textit{Wilson et al.}[12] (2013): 3-6.

[5]	\textbf{Smith}, Catherine L., and Paul B. Kantor. "User adaptation: good results from poor systems." \textit{Proceedings of the 31st annual international ACM SIGIR conference on Research and development in information retrieval}. ACM, 2008.

[6]	\textbf{Turpin}, Andrew H., and William Hersh. "Why batch and user evaluations do not give the same results." \textit{Proceedings of the 24th annual international ACM SIGIR conference on Research and development in information retrieval}. ACM, 2001.

[7]	\textbf{Sinervo}, Barry ÒOptimal Foraging Theory: Constraints and Cognitive Processes" 2006

[8]	\textbf{Beck}, Kent; et al, 2001. Manifesto for Agile Software Development. Agile Alliance. Available at: http://agilemanifesto.org/  

[9]	\textbf{Neilsen}, J. \& Molich, R., 1990. Heuristic evaluation of user interfaces. CHI 90 \textit{Proceedings of the SIGCHI Conference on Human Factors in Computing Systems}, pp. 249-256.

[10]	\textbf{Azzopard}, Leif , Martin Bevc, Andrew Gardner, David Maxwell and Abdullah RazzoukPagefetch 2: Gamification the Sequel, 2010.


\section{technologies}

\textbf{Django} (Version 1.5) [Web Framework]. (2013). Retrieved from: https://djangoproject.com 

\textbf{Python} (Version 2.7.6) [Software]. 2014. Retrieved from: https://www.python.org 

\textbf{Bootstrap} (Version 3.2.0) [Software]. (2014). Retrieved from: http://getbootstrap.com 

\textbf{jQuery} (Version 1.10.1) [Software]. (2014) Retrieved from: http://jquery.com 

\textbf{JetBrains PyCharm Professional Edition} (Version 3.4.1) [Computer Software]. Retrieved from: http://www.jetbrains.com/pycharm/ 

\textbf{Animate.css} (Version 3.2.0) [CSS3 Plugin], by Daniel Eden  available at:  http://daneden.github.io/animate.css/

\textbf{AnimateNumbers.js} [Javascript Plugin] by talmand available at: https://github.com/talmand/jquery-animate-numbers

\textbf{Tiled: map editor} [Software] available at: http://www.mapeditor.org/

\textbf{Photoshop CS6} [Image Editing Software] available at http://www.adobe.com/uk/products/photoshop.html
